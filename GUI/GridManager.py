...
class FrameCell(Frame):
    ...
    def __init__(self, parent, obj, width=200, height=200):
        ...
        self.object = obj
        img = obj.image.resize((int(width*0.75), int(height*0.75)), Image.ANTIALIAS)
        self.image = ImageTk.PhotoImage(img)
        self.img_label = Label(self, image=self.image, bg='white', borderwidth=0)
        self.img_label.grid(row=0, column=0)
        self.heading = Label(self, width=20,  text=obj.text, fg='black', bg='white', font=('Microsoft YaHei UI Light', 12, 'bold'))
...
class RowManager(Frame):
    def __init__(self, parent, objects, row_width=200, column_width=200):
        ...
        for i in range(len(objects)):
            cell = FrameCell(self, obj=objects[i], width=column_width, height=row_width)
...
class GridManager(Frame):
    def __init__(self, parent, column_count, row_width, column_width, height, width, objects):
        ...
        for row in chunker(objects, column_count):
            RowManager(
                self.scrollable_frame.scrollable_frame, row_width=row_width, column_width=column_width, objects=row
            ).pack()
